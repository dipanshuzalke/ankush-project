...
export default function Dashboard() {
  ...
  const [activeRoute, setActiveRoute] = useState<string | null>(pathname ?? null);
  ...
  // keep activeRoute in sync with pathname changes
  useEffect(() => {
    setActiveRoute(pathname ?? null);
  }, [pathname]);
      ...
      ${
        activeRoute?.startsWith("/dashboard")
          ...
          : "bg-[#2a2a2a] text-white" // inactive → black
            ...
            <button
              onClick={() => {
                setActiveRoute('/create-member');
                router.push("/create-member");
              }}
              className={`flex items-center gap-3 px-3 py-2 rounded 
      ${
        activeRoute?.startsWith("/create-member")
